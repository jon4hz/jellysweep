...
func (e *Engine) getEpisodeFilesToKeep(ctx context.Context, series sonarr.SeriesResource, cleanupMode string, keepCount int) ([]int32, error) {
	...
	case CleanupModeKeepEpisodes:
		...
		slices.SortFunc(episodes, func(a, b sonarr.EpisodeResource) int {
		...
		// Keep files for the first keepCount episodes (by episode order)
		keptEpisodes := 0
	...
	case CleanupModeKeepSeasons:
		...
		// Keep files for the first keepCount seasons (lowest-numbered)
		keptSeasons := 0
...
func (e *Engine) unmonitorDeletedEpisodes(ctx context.Context, series sonarr.SeriesResource, cleanupMode string, keepCount int) error {
	...
	switch cleanupMode {
	case CleanupModeKeepEpisodes:
		// Unmonitor episodes that are not in the first N episodes (by episode number)
		// Sort episodes by season number ascending, then by episode number ascending
		slices.SortFunc(episodes, func(a, b sonarr.EpisodeResource) int {
			// Sort by season number ascending (first seasons first)
			if a.GetSeasonNumber() != b.GetSeasonNumber() {
		...
		// Unmonitor episodes beyond the first keepCount episodes
	...
	case CleanupModeKeepSeasons:
		// Unmonitor episodes from seasons that are not in the first N lowest-numbered seasons
		// Group episodes by season
		seasonEpisodes := make(map[int32][]sonarr.EpisodeResource)
		for _, episode := range episodes {
			seasonNum := episode.GetSeasonNumber()
		...
		// Unmonitor episodes from seasons beyond the first keepCount seasons
