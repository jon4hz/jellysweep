...
func (f *Filter) ApplyAll(ctx context.Context, mediaItems []arr.MediaItem) ([]arr.MediaItem, error) {
	...
	for _, filter := range f.filters {
		log.Info("Applying filter to media items.", "filter", filter.String(), "initial_items", len(filteredItems))
		filteredItems, err = filter.Apply(ctx, filteredItems)
		if err != nil {
			log.Error("Failed to apply filter.", "filter", filter.String(), "error", err)
			return nil, err
		}
		log.Info("Filter applied successfully.", "filter", filter.String(), "remaining_items", len(filteredItems))
	}
